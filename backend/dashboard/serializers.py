...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if User.objects.filter(email__iexact=attrs['email'].lower()).exists():
    ...
    def create(self, validated_data):
        ...
        return User.objects.create_user(
            ...
            email=validated_data['email'].lower(),  # ✅ Store email lowercase for consistency
...
class ProfileUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_email(self, value):
        if User.objects.filter(email__iexact=value.lower()).exclude(pk=self.instance.pk).exists():
...
class EventApplicationSerializer(serializers.ModelSerializer):
    ...
    equipment = serializers.CharField(required=False, allow_blank=True, allow_null=True, default='[]')
...
class ApplicationListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model  = EventApplication
        fields = [
            'id', 'event_name', 'event_type', 'event_date', 'start_date', 
            'venue', 'location', 'status', 'status_display', 'created_at',
...
# ==================== ✅ FIXED CUSTOM JWT SERIALIZER ====================
class CustomTokenObtainPairSerializer(TokenObtainPairSerializer):
    """
    Supports login via email (case-insensitive) OR username.
    ...
    def get_token(cls, user):
        ...
        token['organization_role'] = getattr(user, 'organization_role', 'User').upper()
    ...
    def validate(self, attrs):
        # Clean inputs
        email = (attrs.get('email') or '').strip().lower()  # ✅ lowercase for PostgreSQL case-sensitivity
        ...
        # ✅ Find user: email (case-insensitive) OR username
        user = None
        if email:
        ...
        if not user or not user.check_password(password):
            raise serializers.ValidationError({'detail': ['Invalid credentials.']})  # ✅ Array format for consistency
        ...
        # ✅ Return response - ALL values as strings/booleans for JSON compatibility
        return {
            'refresh': str(refresh),
            ...
            'organization_role': getattr(user, 'organization_role', 'User').upper(),
...
class NotificationSerializer(serializers.ModelSerializer):
    class Meta:
        model = Notification
        fields = ['id', 'user', 'notif_type', 'title', 'message', 'is_read', 'created_at', 'payload']
